...
export const computeRoute = async (i, j, notVisited, startingPoint, currentPoint, currentTime, workingHours, truckVel, path, maxNumMunicipisDia, distanceCache, abort = false) => {
  const getDistance = async (origin, destination) => {
    ...
    for (let l = 0; l < nearby.length; ++l) {
      if (notVisited.has(nearby[l].municipiInfo)) {
        ...
        await computeRoute(i, j, notVisited, startingPoint, nearby[l], parseFloat(currentTime + time), workingHours, truckVel, path, maxNumMunicipisDia, distanceCache, abort = true);
        if (abort) return;
